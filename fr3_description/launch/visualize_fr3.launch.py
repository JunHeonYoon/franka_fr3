...
def robot_state_publisher_spawner(context: LaunchContext, robot_side, load_gripper):
    robot_side_str = context.perform_substitution(robot_side).strip().lower()
    load_gripper_str = context.perform_substitution(load_gripper)
    if robot_side_str in ('left', 'right'):
        franka_xacro_filepath = os.path.join(
            get_package_share_directory('fr3_description'),
            'robots',
...
def generate_launch_description():
    ...
    robot_state_publisher_spawner_opaque_function = OpaqueFunction(
        function=robot_state_publisher_spawner, args=[robot_side, load_gripper]
    )
