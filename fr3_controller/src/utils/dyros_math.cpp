#include "fr3_controller/utils/dyros_math.h"
...
)
{
  ...
  {
    double elapsed_time = time - time_0;
    double elapsed_time2 = elapsed_time * elapsed_time; // pow(t,2)
    double elapsed_time3 = elapsed_time2 * elapsed_time; // pow(t,3)
    double elapsed_time4 = elapsed_time3 * elapsed_time; // pow(t,4)
    double elapsed_time5 = elapsed_time4 * elapsed_time; // pow(t,5)
    double total_time = time_f - time_0;
    double total_time2 = total_time * total_time;  // pow(t,2)
    double total_time3 = total_time2 * total_time; // pow(t,3)
    double total_time4 = total_time3 * total_time; // pow(t,4)
    double total_time5 = total_time4 * total_time; // pow(t,5)
    double total_x = x_f - x_0;

    //Quintic polynomial
    x_t = x_0
      + x_dot_0 * elapsed_time
      + (x_ddot_0 / 2) * elapsed_time2
      + ((20 * total_x - (8 * x_dot_f + 12 * x_dot_0) * total_time - (3 * x_ddot_0 - x_ddot_f) * total_time2) / (2 * total_time3)) * elapsed_time3
      + ((-30 * total_x + (14 * x_dot_f + 16 * x_dot_0) * total_time + (3 * x_ddot_0 - 2 * x_ddot_f) * total_time2) / (2 * total_time4)) * elapsed_time4
      + ((12 * total_x - (6 * x_dot_f + 6 * x_dot_0) * total_time - (x_ddot_0 - x_ddot_f) * total_time2) / (2 * total_time5)) * elapsed_time5;
  }
  return x_t;
}
...

{
  ...
  else
  {
    ...
    x_t = x_dot_0
      + 2 * (x_ddot_0 / 2) * elapsed_time
      + 3 * ((20 * total_x - (8 * x_dot_f + 12 * x_dot_0) * total_time - (3 * x_ddot_0 - x_ddot_f) * total_time2) / (2 * total_time3)) * elapsed_time2
      + 4 * ((-30 * total_x + (14 * x_dot_f + 16 * x_dot_0) * total_time + (3 * x_ddot_0 - 2 * x_ddot_f) * total_time2) / (2 * total_time4)) * elapsed_time3
      + 5 * ((12 * total_x - (6 * x_dot_f + 6 * x_dot_0) * total_time - (x_ddot_0 - x_ddot_f) * total_time2) / (2 * total_time5)) * elapsed_time4;
  }
  return x_t;
}
  ...
  const Eigen::Matrix3d& rotation_f)
{
  ...
  double tau = quintic(time, time_0, time_f, 0, 1, 0, 0, 0, 0);
  Eigen::Matrix3d rot_scaler_skew;
  rot_scaler_skew = (rotation_0.transpose() * rotation_f).log();
  Eigen::Matrix3d result = rotation_0 * (rot_scaler_skew * tau).exp();
  return result;
...
Eigen::Vector3d rotationQuinticDot(double time,
  double time_0,
  double time_f,
  const Eigen::Matrix3d& rotation_0, const Eigen::Matrix3d& rotation_quintic,
  const Eigen::Matrix3d& rotation_f)
{
  Eigen::Vector3d result;
  Eigen::Vector3d result_e;
  result << 0.0, 0.0, 0.0;
  result_e << 0.0, 0.0, 0.0;
  if (time >= time_f)
  {
    return result;
  }
  ...
  double theta_dot = quinticDot(time, time_0, time_f, 0, theta, 0, 0, 0, 0);
  Eigen::Vector3d w;
  ...
  if(rotation_0 == rotation_f)
  {
    std::cout <<"exception called result : " << result_e.transpose()  << std::endl;
    return result_e;
  }
  else{
    return result;
...
Eigen::Isometry3d rigidRotation(const Eigen::Vector3d& cor, const Eigen::Vector3d& axis, double angle)
	{
    // Return rigid body rotation Isometry3d around axis
    // cor : Center of radius w.r.t {initial EE}
    // axis : Rotation axis w.r.t {initial EE}
    // angle : Rotation angle [rad]
		Eigen::Isometry3d tempA;
		tempA.linear().setIdentity();
		tempA.translation() = cor;

		Eigen::Isometry3d rot;
		rot = Eigen::AngleAxisd(angle, axis);

		Eigen::Isometry3d tempB;
		tempB.linear().setIdentity();
		tempB.translation() = -cor;

		Eigen::Isometry3d result;
		result = tempA.matrix() * rot.matrix() * tempB.matrix();
		return result;
	}
