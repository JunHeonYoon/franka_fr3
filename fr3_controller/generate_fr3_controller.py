...
Generates FR3 controller boilerplate:
  - HPP: include/fr3_controller/<snake>.hpp
  - CPP: src/<snake>.cpp

Also patches:
  - CMakeLists.txt        : adds .cpp to add_library block
  - fr3_plugin.xml        : adds class entry
...
#include "fr3_controller/fr3_controller_parameters.hpp"

#include <fr3_controller/utils/dyros_math.h>
#include <fr3_controller/utils/robot_utils.hpp>
...
def render_cpp(class_name: str, snake: str) -> str:
    return f"""#include "fr3_controller/{snake}.hpp"
...
// NOLINTNEXTLINE
PLUGINLIB_EXPORT_CLASS(fr3_controller::{class_name}, controller_interface::ControllerInterface)
"""
...
def patch_plugin_xml(xml_path: Path, class_name: str) -> Tuple[bool, str]:
    ...
    full_class = f"fr3_controller/{class_name}"

    if full_class in text:
        return False, f"Already present: {full_class}"

    new_entry = (
        f'\n  <class name="fr3_controller/{class_name}"\n'
        f'         type="fr3_controller::{class_name}"\n'
...
def patch_yaml(yaml_path: Path, snake: str, class_name: str, control_mode: str) -> Tuple[bool, str]:
    ...
    type_pattern = re.compile(r"^ {8}type: fr3_controller/\S+", re.MULTILINE)
    matches = list(type_pattern.finditer(text))

    new_type_entries = (
        f"\n\n      {left_name}:\n        type: fr3_controller/{class_name}"
        f"\n\n      {right_name}:\n        type: fr3_controller/{class_name}"
        f"\n\n      {dual_name}:\n        type: fr3_controller/{class_name}"
    )
...
def main() -> None:
    ...
    xml_path   = pkg_root / "fr3_plugin.xml"
    ...
    hpp_path = pkg_root / "include" / "fr3_controller" / f"{snake}.hpp"
    ...
    print(f"  - {cmake_msg}")
    print("fr3_plugin.xml:")
    print(f"  - {xml_msg}")
    print("config/fr3_ros_controllers.yaml:")
    print(f"  - {yaml_msg}")

    print(f"\nController class : fr3_controller::{class_name}")
    print(f"Plugin type      : fr3_controller/{class_name}")
    ...
    print("  2) Build: colcon build --packages-select fr3_controller")
